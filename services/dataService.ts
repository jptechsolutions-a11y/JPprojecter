...
export const api = {
  fetchProjectData: async (requestedTeamId: string | null) => {
    try {
      ...
      const { data: teamUsersData } = await supabase.from('team_members').select('user_id, profiles(*)').eq('team_id', teamId);
      const mappedUsers = teamUsersData ? teamUsersData.map((tu: any) => mapUser(tu.profiles)).filter(u => u.id) : [];
